...
<head>
	<%
		...
		List<mDto> userSearchList = (List<mDto>)request.getAttribute("userList");
...
<script type="text/javascript">
	$(function(){
		...
		$("#jForm").submit(function(){
			...
			var search = $(".search").eq(0).children("option").length;
			for(var i = 0; i < search; i++){
				if($(".typeOption").eq(i).val() == "<%=type %>"){
					$(".typeOption").eq(i).prop("selected","selected");
				}
			}
	...
	function select(){
		...
		location.href="metroCon.do?command=search&type=<%=type %>&keyword=<%=keyword %>&pageNumber=1&rowSize="+rowSize
	};
...
<body>
	...
	<div id="wrap">		
				...
				<c:forEach items="${userList }" var="userList">
					...
					<tr>
						...
						<select class="role">
							...
							<option class="roleOption">${anotherRole }</option>
						</select>
						...
						<select class="enabled">
							...
							<option class="enabledOption">${anotherEnabled }</option>
						</select>
						</td>
		...
		<section id="pageWrap" style="margin-bottom:150px;">
			<span class="pgNum"><a href="metroCon.do?command=search&pageNumber=1&rowSize=<%=rowSize %>&type=<%=type %>&keyword=<%=keyword %>">&lt;</a></span>
			<%
			...
			%>
				<span class="pgNum"><a href="metroCon.do?command=search&pageNumber=<%=i %>&rowSize=<%=rowSize %>&type=<%=type %>&keyword=<%=keyword %>"><%=i %></a></span>
				
			<%
				}
			%>
			<span class="pgNum"><a href="metroCon.do?command=search&pageNumber=<%=pageSize %>&rowSize=<%=rowSize %>&type=<%=type %>&keyword=<%=keyword %>">&gt;</a></span>
		</section>
		<!--// 페이징 -->
